...
export default async function handler(req, res) {
  ...
  // ── probe_batch — called repeatedly by browser, 3 guesses per call (~3s each)
  // Replaces monolithic probe to stay under Vercel free tier 10s function limit.
  // Browser drives pagination: sends batch=0, batch=1, ... until done=true.
  if (action === 'probe_batch') {
    const ALL_GUESSES = [
    ...
    const BATCH_SIZE  = 3;   // 3 guesses × 1s spacing = ~3s per call, well under 10s
    const TOTAL_BATCHES = Math.ceil(ALL_GUESSES.length / BATCH_SIZE);

    const batchIndex = Math.max(0, parseInt(req.query.batch || '0', 10));
    if (batchIndex >= TOTAL_BATCHES) {
      return res.status(200).json({ ok: true, results: [], done: true, daily_remaining: LIMITS.DAILY_MAX - dailyCount });
    }

    // Cooldown only on first batch
    if (batchIndex === 0) {
      const now = Date.now();
      ...
      probeLastRun = Date.now();
    }

    const batch = ALL_GUESSES.slice(batchIndex * BATCH_SIZE, (batchIndex + 1) * BATCH_SIZE);

    if (dailyCount + batch.length > LIMITS.DAILY_MAX) {
      return res.status(429).json({
        ok: false, type: 'rate_limit',
        error: `Not enough daily quota (need ${batch.length}, have ${LIMITS.DAILY_MAX - dailyCount})`,
      });
    }
    ...
    try {
      ...
      for (const { entity, dataset } of batch) {
        if (!slidingWindowCheck(globalWindow, LIMITS.GLOBAL_PER_MIN)) {
        ...
        // 1s between calls within the batch
        if (batch.indexOf({ entity, dataset }) < batch.length - 1) {
          await new Promise(r => setTimeout(r, 1000));
        }
      }

      const done = (batchIndex + 1) >= TOTAL_BATCHES;
      return res.status(200).json({
        ok:              true,
        results,
        batch:           batchIndex,
        next_batch:      done ? null : batchIndex + 1,
        done,
        progress:        `${(batchIndex + 1) * BATCH_SIZE}/${ALL_GUESSES.length}`,
        daily_remaining: LIMITS.DAILY_MAX - dailyCount,
      });
    } catch (err) { return res.status(502).json({ ok: false, error: err.message }); }
  }
  ...
  return res.status(400).json({
    ...
    valid_actions: ['token', 'status', 'health', 'query', 'probe_batch'],
  });
}
